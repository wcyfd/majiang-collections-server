...
public class NewHu extends Hu {
    ...
    // public boolean find(List<Integer> oneList, Set<Integer> useIndexSet, int
    // startIndex, int baidaCount,
    // Ref<Integer> needCountRef) {
    // int index = findUnuseCardIndex(oneList, useIndexSet, startIndex,
    // oneList.size(), oneList.get(startIndex));
    // if (needCountRef.get() > baidaCount) {
    // return false;
    // }
    // // 找吃
    // Set<Integer> set1 = new HashSet<>();
    // set1.addAll(useIndexSet);
    //
    // Ref<Integer> needCountRef1 = new Ref<>();
    // needCountRef1.set(needCountRef.get());
    // findChi(oneList, oneList.get(index), set1, index, needCountRef1);
    // if (needCountRef.get() <= baidaCount) {
    // int index1 = findUnuseCardIndex(oneList, useIndexSet, startIndex,
    // oneList.size(), oneList.get(startIndex));
    // findPeng(oneList, oneList.get(index), useIndexSet, index1,
    // needCountRef1);
    // if()
    // } else {
    // findPeng(oneList, oneList.get(index), useIndexSet, index, needCountRef);
    // if (needCountRef.get() <= baidaCount) {
    // }
    // }
    //
